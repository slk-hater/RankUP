...
public class Miner {
    ...
    static String DESCRIPTION = "&7Tipo &f%type%\n&7Velocidade &f{speed}/s\n\n&8Botão direito no chão para posicionar";
    ...
    public void setType(MinerType minerType) {
        ...
        if(minerType.equals(MinerType.ADVANCED)) translatedType = "AVANÇADO";
        else translatedType = "BÁSICO";
        String blocks = speed == 1 ? "bloco" : "blocos";

        ItemMeta meta = getItemStack().getItemMeta();
        if(meta == null) return;
        String lore = DESCRIPTION.replace("%type%", translatedType).replace("%speed%", getSpeed()+blocks);
